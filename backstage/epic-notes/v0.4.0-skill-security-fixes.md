...
**Problem:**
- `checks.sh` reads `$HOME/Documents/backstage/backstage/global/POLICY.md`
...
**Problem:**
- `checks.sh` extracts code blocks from `HEALTH.md`
- Executes them using `eval "$check"`
- No sandboxing, no interpreter restrictions
- Any bash command in HEALTH.md runs with full user permissions

**Risk:**
- Malicious code in HEALTH.md executes automatically
- User may not realize HEALTH.md = executable script
- High blast radius (full shell access)

**Scan Quote:**
> "checks.sh extracts code blocks from PROJECT/GLOBAL HEALTH.md and executes them using eval. Executing arbitrary code from README/HEALTH/POLICY files is powerful and can run any shell command; while this is part of the declared 'executable rules' model, it significantly increases risk and is not explicitly warned in metadata or SKILL.md as a dangerous capability."
...
> 1. **Inspect HEALTH.md** and any global POLICY/HEALTH files for embedded bash code — those code blocks will be executed.
...
- [ ] **Add security warning** in README/SKILL.md: "HEALTH.md code blocks execute with full shell access"
...
- [ ] **Explicit permission model** (user approves HEALTH.md execution before first run?)
...
**Current design:**
- HEALTH.md = executable tests (bash code blocks)
- POLICY.md = interpretive rules (AI reads + acts)
- Polycentric governance (global + project, project wins)
- Auto-sync docs with reality (anti-drift)

**Security tradeoff:**
- Power = flexibility (execute anything in HEALTH.md)
- Risk = trust (user must audit HEALTH.md before running)
...
- HEALTH.md = only AI-interpretable prose
...
- Keep eval for project-local HEALTH.md (user writes it, they own risk)
- **Remove** eval for global HEALTH.md (too much power for shared file)
