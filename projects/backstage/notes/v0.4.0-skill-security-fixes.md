...
- **Claim:** "Installs protocol if missing"
- **Reality:** No install spec, no fetch/install mechanism
- **Risk:** Assumes external artifacts exist (undefined behavior)

### 2. GLOBAL CHECKS EXECUTION
- **Behavior:** Executes ALL `*.sh` from `$HOME/Documents/backstage/backstage/checks/global/`
- **Risk:** Arbitrary code execution outside project scope
- **Blast radius:** User's home directory (read files, network calls, file writes)
- **Disproportionate:** Workflow-checking skill shouldn't execute home directory scripts

### 3. INSTALL MECHANISM INCOHERENCE
- **`requires:`** Lists GitHub repo URL
- **No installer:** URL is NOT an installation instruction
- **Confusion:** SKILL.md says "installs" but nothing performs fetch/install

### 4. AUTONOMOUS INVOCATION RISK
- **Skill allows:** Agent can invoke autonomously (default)
- **Combined risk:** Auto-invoke + global checks = automatic home directory script execution

### 5. contract.html HTTP SERVER
...
### Immediate (v1.0.1 fixes)

- [ ] **Remove "installs protocol" claim** - skill doesn't install, it assumes protocol exists
- [ ] **Document global checks path** - explain WHY $HOME/Documents/backstage (not arbitrary)
- [ ] **Add install instructions** - separate from skill, explicit setup docs
- [ ] **Warn about autonomous invocation** - document blast radius

### Future (v1.1.0+)

- [ ] **Repo-local checks preference** - prioritize project checks/ over global
- [ ] **Explicit consent for global checks** - ask before executing $HOME scripts
...
**Is global checks/ legitimate?**

**YES:**
- Backstage = protocol (like git, not app)
- Global checks = shared rules across projects (DRY)
- User SHOULD control $HOME/Documents/backstage (not arbitrary)

**BUT:**
- Skill must DOCUMENT this clearly
- User must OPT-IN explicitly
- Install process must be transparent
